...
if (!err) {
    auto savefile_variant_length = tmp_ver[0];
    auto variant_length = static_cast<char>(VARIANT_NAME.length());
    if (savefile_variant_length == variant_length) {
        if (std::string_view(&tmp_ver[1], variant_length) != VARIANT_NAME) {
            throw(_("セーブデータのバリアントはtangband以外です", "The variant of save data is other than ABCband!"));
        }
